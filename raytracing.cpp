...
Sphere scene[5] = {
    ...
    Sphere({2, 0, 4}, 1, {0, 0, 255}, 500, 0.3, 0.7),
    Sphere({-2, 0, 4}, 1, {0, 255, 0}, 10, 0.4, 0.9, 2.42),
    Sphere({0, -5001, 0}, 5000, {255, 255, 0}, 1000, 0.5),
    Sphere({3, 4, 8}, 2, {255, 0, 255}, 1000, 0.5)
};
...
Triangle sceneTriangles[2] = {
    Triangle({0,0,2}, {1,2,2}, {-1,2,2}, {255,0,255}, 750, 0.5, 0.5, 1.5), // Semi-transparent
...
Vector RefractRay(Vector I, Vector N, double n1, double n2) { // new
    double n = n1 / n2;
    double cosI = -dot(N, I);
...
Vector TraceRay(const Vector& O, const Vector& D, double t_min, double t_max, int depth) {
    ...
    if (transparency > 0) {
        double currentRefractiveIndex = 1.0; // Assume air as the initial medium
        Vector R = RefractRay(V, N, currentRefractiveIndex, refractiveIndex);
